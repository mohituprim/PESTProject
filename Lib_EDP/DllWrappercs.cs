...
namespace FactSet.Partners.Lib_EDP
{
    ...
    public class DllWrapper
    {
        public DllWrapper(string dllShortNameWithoutExt)
        {
            ...
            FileIOPermission fileRights = new FileIOPermission(PermissionState.Unrestricted)
            {
            ...
            permSet.AddPermission(new SqlClientPermission(PermissionState.Unrestricted));
            permSet.AddPermission(new WebPermission(PermissionState.Unrestricted));

            AppDomain newDomain = AppDomain.CreateDomain("Sandbox", evidence, adSetup, permSet, StrongNamesFromDllPath());
        ...
        private StrongName[] StrongNamesFromDllPath()
        {
            return new StrongName[] {
                typeof(DBOperations.SQLServerOperations).Assembly.Evidence.GetHostEvidence<StrongName>()
            };
        }
